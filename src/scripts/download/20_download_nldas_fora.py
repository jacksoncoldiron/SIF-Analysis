...
the full CONUS domain and extracts the variables needed to compute:
  ...
  - Tair  — 2-m air temperature (K)                → variable 'TMP'
  - Qair  — specific humidity (kg/kg)               → variable 'SPFH'
  - PSurf — surface pressure (Pa)                   → variable 'PRES'
  - Wind_E — zonal wind component (m/s)             → variable 'UGRD'
  - Wind_N — meridional wind component (m/s)        → variable 'VGRD'
  - SWdown — downward shortwave radiation (W/m²)    → variable 'DSWRF'
...
Output:
  ...
  data/processed/conus/nldas_fora/FORA_YYYYMM.nc         — clipped to CONUS analysis grid
      Variables: TMP, SPFH, PRES, UGRD, VGRD, DSWRF
"""
...
# FORA variables to extract
FORA_VARS = ['TMP', 'SPFH', 'PRES', 'UGRD', 'VGRD', 'DSWRF']
